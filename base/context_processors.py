...
def update_selected_company(request):
    ...
    if company_scoped_active() and not request.user.is_superuser:
        ...
        if not target_allowed:
            ...
            return SolichRedirect(request, redirect_to=next_url)
...
def white_labelling_company(request):
    ...
    if white_labelling:
        ...
        return {
            "white_label_company_name": "Solich",
...
def doc_base_url(request):
    ...
    """
    return {
        "DOC_BASE_URL": getattr(settings, "DOC_BASE_URL", "https://www.solich.com")
    }
...
def resignation_request_enabled(request):
    ...
    if apps.is_installed("offboarding"):
        OffboardingGeneralSetting = get_solich_model_class(
...
def timerunner_enabled(request):
    ...
    if apps.is_installed("attendance"):
        AttendanceGeneralSetting = get_solich_model_class(
...
def intial_notice_period(request):
    ...
    if apps.is_installed("payroll"):
        PayrollGeneralSetting = get_solich_model_class(
...
def check_candidate_self_tracking(request):
    ...
    if apps.is_installed("recruitment"):
        RecruitmentGeneralSetting = get_solich_model_class(
