...
from solich_views.cbv_methods import login_required, paginator_qry
from solich_views.generic.cbv.views import SolichCardView, SolichNavView
...
@method_decorator(login_required, name="dispatch")
class RosterNavView(SolichNavView):
...
@method_decorator(login_required, name="dispatch")
class RosterGridView(SolichCardView):
    model = Roster
            ...
            for col_i, d in enumerate(date_range, start=3):
                ...
                if off_reason:
                    ...
                    if off_reason != "Weekly Off Day":
                        c.comment = Comment(f"Holiday: {off_reason}", "Solich")
                elif d.weekday() >= 5:
                    c.fill = weekend_fill
