...
class AnnouncementFormView(SolichFormView):
    """
    ...
    def form_valid(self, form: AnnouncementForm) -> HttpResponse:
        if form.is_valid():
            ...
            emp_dep = SolichUser.objects.filter(
                employee_get__employee_work_info__department_id__in=departments
            )
            emp_jobs = SolichUser.objects.filter(
...
@method_decorator(login_required, name="dispatch")
class AnnouncementDetailView(SolichDetailedView):
