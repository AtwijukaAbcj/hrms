...
Sections registered:
  ...
  - Appearance : color theme (only when solich_theme is installed)
...
def system_preferences_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            ...
            "solich_audit.view_accountblockunblock",
            "employee.change_employeegeneralsetting",
            "solich_audit.view_historytrackingfields",
...
def history_tags_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            ...
            "solich_audit.view_audittag",
        ]
    )


def audit_tracking_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("solich_audit.view_auditmodelconfig")
...
def audit_history_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            "solich_audit.view_audittag",
            "solich_audit.view_auditmodelconfig",
        ]
    )
...
def mail_template_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("base.view_solichmailtemplate")


def mail_automation_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("solich_automations") and request.user.has_perm(
        "solich_automations.view_mailautomation"
    )
...
def color_theme_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("solich_theme.view_solichcolortheme")


def gdrive_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("solich_backup.view_googledrivebackup")
...
def ldap_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("solich_ldap") and any(
        request.user.has_perm(p)
        for p in ["solich_ldap.add_ldapsettings", "solich_ldap.update_ldapsettings"]
    )


def google_meet_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("solich_meet") and request.user.has_perm(
        "solich_meet.view_googlecloudcredential"
    )
...
# ---------------------------------------------------------------------------
# 5. Appearance section (only when solich_theme is installed)
# ---------------------------------------------------------------------------
...
class ThemeManagerSettings:
    title = _("Appearance")
    order = 10
    condition = lambda self, request: apps.is_installed("solich_theme")
    items = [
        {
            "label": _("Color Theme"),
            "url": reverse_lazy("solich_theme:color_theme_view"),
