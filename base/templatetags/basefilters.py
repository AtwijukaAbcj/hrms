...
from solich.menu.settings_menu import get_settings_menu
...
def _get_employee_of_user(user):
    ...
    """
    if not hasattr(user, "_solich_employee_cache"):
        user._solich_employee_cache = Employee.objects.filter(
            employee_user_id=user
        ).first()
    return user._solich_employee_cache
...
def is_manager_of(user, instance, field_name="employee_id"):
    ...
    if not hasattr(user, "_solich_is_manager_of_cache"):
        user._solich_is_manager_of_cache = {}
    cache = user._solich_is_manager_of_cache
...
def is_leave_approval_manager(user):
    ...
    """
    if hasattr(user, "_solich_is_leave_approval_manager_cache"):
        return user._solich_is_leave_approval_manager_cache
    employee = _get_employee_of_user(user)
    manager = (
    ...
    user._solich_is_leave_approval_manager_cache = manager
    return manager
...
def general_section_main(context):
    ...
    return any(
        [
            ...
            user.has_perm("solich_audit.view_accountblockunblock"),
            ...
            user.has_perm("solich_audit.view_historytrackingfields"),
            ...
            user.has_perm("solich_audit.view_audittag"),
            user.has_perm("base.view_dynamicemailconfiguration"),
            user.has_perm("solich_backup.view_googledrivebackup"),
        ]
    )
...
def show_section(context):
    ...
    return any(
        [
            ...
            user.has_perm("solich_audit.view_audittag"),
            user.has_perm("solich_backup.view_googledrivebackup"),
            user.has_perm("solich_ldap.add_ldapsettings"),
            user.has_perm("solich_ldap.update_ldapsettings"),
            user.has_perm("employee.view_actiontype"),
            user.has_perm("base.view_tags"),
            ...
            user.has_perm("solich_ldap.add_ldapsettings"),
            user.has_perm("solich_ldap.update_ldapsettings"),
            user.has_perm("solich_meet.view_googlecloudcredential"),
            user.has_perm("whatsapp.add_whatsappcredientials"),
            user.has_perm("solich_theme.view_solichcolortheme"),
        ]
    )
...
def settings_search_index():
    ...
    from solich.menu.settings_menu import settings_registry
...
def config_perms(user):
    ...
    app_permissions = {
        ...
        "base": [
            ...
            "base.add_solichmailtemplates",
            "base.view_solichmailtemplates",
        ],
    }
