...
def checkminimumot(ot=None):
    ...
    if ot is not None:
        if apps.is_installed("attendance"):
            AttendanceValidationCondition = get_solich_model_class(
...
def _resolve_company_theme(company_id):
    if company_id is not None and company_id != "all":
        ...
        if theme:
            return SolichColorTheme.objects.filter(id=theme.theme.id).first()
        else:
...
def get_company(context):
    ...
    cache = getattr(request, "_solich_company_theme_cache", None)
    if cache is None:
        cache = {}
        request._solich_company_theme_cache = cache
