...
from solich.decorators import apply_decorators
...
def get_diff(instance, history_related_name):
    ...
    for pair in pairs:
        ...
        if not diffs:
            continue
        updated_by = (
            SolichUser.objects.get(id=pair[0].history_user.id).employee_get
    ...
    # simple-history) have no rows — still surface creation from SolichModel.
    ...
    if instance._meta.model_name == "employeeworkinformation":
        from solich_audit.models import HistoryTrackingFields
...
def history_tracking(request, obj_id, **kwargs):
    ...
    def _history_tracking(request, obj_id, model):
        ...
        histories = instance.solich_history.all()
        ...
        return render(
            request,
            "solich_audit/history_tracking.html",
            context,
        )
