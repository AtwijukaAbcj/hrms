...
from solich.solich_middlewares import _thread_locals, get_selected_company
from solich.http.response import SolichRedirect
from solich.models import SolichModel
from solich.signals import post_generic_import, pre_generic_import
from solich_views import models
from solich_views.cbv_methods import (  # update_initial_cache,
    assign_related,
    export_xlsx,
...
from solich_views.forms import DynamicBulkUpdateForm, ToggleColumnForm
from solich_views.related_link_registry import RELATED_VIEW_PARAM, register_detail_view
...
def model_has_audit_tracking(model) -> bool:
    ...
    ``solich_history`` field every ``SolichModel`` subclass gets.
    """
    return (
...
class SolichListView(ListView):
    ...
    template_name: str = "generic/solich_list_table.html"
    ...
    def get_context_data(self, **kwargs: Any):
        ...
        if (
            ...
            and issubclass(self.model, SolichModel)
        ...
        if request and self._saved_filters.get("field"):
            ...
            try:
                ...
                self.template_name = "generic/solich_list_table.html"
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[SolichListView] = context
        self.export_path = (
...
class SolichDetailedView(DetailView):
    ...
    # target for `model` (see solich_views/related_link_registry.py), instead
...
class SolichTabView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[SolichTabView] = context
...
class SolichCardView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[SolichCardView] = context
        referrer = self.request.GET.get("referrer", "")
        if referrer:
            # Remove the protocol and domain part
            referrer = "/" + "/".join(referrer.split("/")[3:])
        ...
        if not isinstance(existing_form, forms.BaseForm):
            ...
            if (
                ...
                if isinstance(company_field, forms.ModelMultipleChoiceField):
                    ...
                    form.initial["company_id"] = selected_company
            # CACHE.get(self.request.session.session_key + "cbv")[SolichFormView] = form
            self.form = form
        return self.form
...
class SolichNavView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[SolichNavView] = context
        return context
...
@method_decorator(hx_request_required, name="dispatch")
class SolichProfileView(DetailView):
    """
    GenericSolichProfileView
    """

    template_name = "generic/solich_profile_view.html"
    view_id: str = None
    filter_class: FilterSet = None
    push_url: str = None
    key_name: str = "pk"
    ...
    # solich_views/urls.py — see dispatch_profile_tab() below for why this
    ...
    def _register_tabs(cls) -> None:
        ...
        for tab in cls.tabs:
            if not tab.get("url"):
                ...
                SolichProfileView._tab_view_registry[key] = tab["view"]
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request = request
        self.ordered_ids_key = f"ordered_ids_{self.model.__name__.lower()}"
        # update_initial_cache(request, CACHE, SolichProfileView)
...
def dispatch_profile_tab(request, tab_key: str, pk: int, *args, **kwargs):
    ...
    solich_views/urls.py) that every SolichProfileView tab (candidate
    profile, employee profile, etc.) resolves through.

    SolichProfileView used to append a brand new URL pattern to the live
    ...
    in SolichProfileView._tab_view_registry at request time removes both
    problems: the registry is fully populated before Django serves its first
    request (see SolichProfileView._register_tabs), the same as it would be
    in every worker process.
    """
    view_func = SolichProfileView._tab_view_registry.get(tab_key)
