...
var steps = [
    { popover: { title: gettext('Dashboard'), description: gettext('Solich dashboard section') } },
...
if ($('.oh-card-dashboard').length) {
    steps.push({ element: '#tileContainer .oh-card-dashboard:nth-child(1)', popover: { title: gettext('Dashboard Tiles'), description: gettext('Solich Dashboard Tiles') } });
}
setTimeout(() => {
    ...
    if ($('[data-id="dashboardNav"]').length) {
        steps.push({ element: '[data-id="dashboardNav"]', popover: { title: gettext('App'), description: gettext('Solich Hr Apps. eg Dashboard') } });
    }
}, 1000);
driverObj = driver(
