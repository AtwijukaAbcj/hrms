...
function safeInterpolate(message, context, named) {
    if (typeof interpolate === 'function') {
        return interpolate(message, context, named);
    }
    ...
    if (named) {
        return message.replace(/%(\w+)s/g, function (_, key) {
            return Object.prototype.hasOwnProperty.call(context, key) ? String(context[key]) : `%(${key})s`;
        });
    }
    return message.replace(/%\((\w+)\)s/g, function (_, key) {
        return Object.prototype.hasOwnProperty.call(context, key) ? String(context[key]) : `%(${key})s`;
...
if (typeof i18nMessages === 'undefined') {
    var i18nMessages = {
        ...
        confirm: safeGettext("Confirm"),
        close: safeGettext("Close"),
        cancel: safeGettext("Cancel"),
        selected: safeGettext("Selected"),
        uploading: safeGettext("Uploading..."),
        emptyMessages: safeGettext("No Records found"),
        ok: safeGettext("Ok"),
        downloadExcel: safeGettext("Do you want to download the excel file?"),
        downloadTemplate: safeGettext("Do you want to download the template?"),
        noRowsSelected: safeGettext("No rows are selected from the records."),
        confirmBulkDelete: safeGettext("Do you really want to delete all the selected records?"),
        confirmBulkArchive: safeGettext("Do you really want to archive all the selected records?"),
        confirmBulkReject: safeGettext("Do you really want to approve all the selected requests?"),
        confirmBulkApprove: safeGettext("Do you really want to approve all the selected requests?"),
        confirmBulkUnArchive: safeGettext("Do you really want to unarchive all the selected records?"),
        totalVacancy: safeGettext("Total vacancy is %(vacancy)s."),
        candidateStageChange: safeGettext(
...
/**
 * Clear list-row selections when switching Solich tabs. Lists may use a
...
// checked in solich_list_table.html / group_by_table.html's
...
function toggleColumnVisibility(checkboxEl, fieldName, visible) {
    // Scope to this list only. SolichTabView keeps visited tabs in the DOM,
