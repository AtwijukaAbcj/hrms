...
from solich.decorators import owner_can_enter
from solich_api.api_decorators.base.decorators import permission_required
from solich_api.api_methods.employee.methods import get_next_badge_id
...
class DocumentRequestAPIView(APIView):
    ...
    def get(self, request, pk=None):
        if pk:
            ...
            has_perm = request.user.has_perm("solich_documents.view_documentrequest")
            ...
            if request.user.has_perm("solich_documents.view_documentrequest"):
    ...
    @manager_permission_required("solich_documents.add_documentrequest")
    def post(self, request):
    ...
    @manager_permission_required("solich_documents.change_documentrequests")
    def put(self, request, pk):
        document_request = self.get_object(pk)
...
class DocumentAPIView(APIView):
    ...
    def get_object(self, pk, request=None):
        ...
        if request is not None and not can_access_document(
            request, document, "solich_documents.view_document"
        ):
    ...
    def get(self, request, pk=None):
        if pk:
            ...
            employee = request.user.employee_get
            if request.user.has_perm("solich_documents.view_document"):
    ...
    @manager_or_owner_permission_required(
        DocumentRequest, "solich_documents.add_document"
    )
    def post(self, request):
    ...
    @method_decorator(owner_can_enter("solich_documents.change_document", Employee))
    def put(self, request, pk):
        document = self.get_object(pk, request)
    ...
    @method_decorator(owner_can_enter("solich_documents.delete_document", Employee))
    def delete(self, request, pk):
...
class DocumentRequestApproveRejectView(APIView):
    ...
    @manager_permission_required("solich_documents.add_document")
