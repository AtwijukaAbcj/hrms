...
Solich takes security seriously. This document explains how to report vulnerabilities, which versions we support, what is in or out of scope, and how we handle disclosure and CVE assignment.

This policy applies to Solich HR ([`solich/solich-hr`](https://github.com/solich/solich-hr)).
...
See [Discussion #1127](https://github.com/solich/solich-hr/discussions/1127) for background on this policy.
...
1. Open a [private vulnerability report](https://github.com/solich/solich-hr/security/advisories/new) on this repository.
...
- Affected Solich HR **version** or commit / Docker tag
...
- Vulnerabilities in **Solich application code** shipped in this repository
...
We will normally **not** treat the following as Solich product CVEs. We may still harden or document them when useful.

| Class | Notes |
|-------|--------|
| CSV / Excel formula injection | Spreadsheet clients interpret cell content; not a Solich application bug |
| Privilege escalation by users who already administer users/roles | Trusted-admin capability by design |
| Issues only on EOL Python or EOL Solich versions | Upgrade to a supported line |
...
| Dependency CVEs with **no reachable path** in Solich | Tracked via Dependabot when applicable |
| Third-party plugins or custom code not shipped by Solich | Report to that project’s maintainers |
...
We use **GitHub as the CVE Numbering Authority** for Solich HR advisories. We do not require reporters to self-request CVEs from MITRE; unsupported self-requests may be disputed.
...
- Is in **Solich code** or an unsafe default we ship
...
- Security reports: [GitHub Private Vulnerability Reporting](https://github.com/solich/solich-hr/security/advisories/new) only — see [Reporting a vulnerability](#reporting-a-vulnerability)
...
The Solich project and its maintainers assume no liability for security vulnerabilities reported or discovered. We greatly appreciate responsible disclosure that helps keep users safe.
