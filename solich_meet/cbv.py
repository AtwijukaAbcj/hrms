...
from solich_meet.filters import GoogleMeetingFilter
from solich_meet.form import GoogleCloudCredentialForm, GoogleMeetingForm
from solich_meet.models import GoogleCloudCredential, GoogleCredential, GoogleMeeting
...
class GMeetCredentialSectionView(views.SolichSectionView):
    """
    View for displaying the Google Meet credentials section.

    Inherits from `SolichSectionView` to manage the section for Google Cloud credentials in the user interface.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(check_integration_enabled(app_name="solich_meet"), name="dispatch")
@method_decorator(
    permission_required("solich_meet.view_googlecloudcredential"), name="dispatch"
)
class GmeetCredentialNavView(views.SolichNavView):
    """
    View for the Google Meet credentials nav view.

    Inherits from `SolichNavView` to manage nav view for Google Cloud credentials. Handles dynamic attributes
...
class GmeetCredentialListView(views.SolichListView):
    """
    View for listing Google Meet credentials.

    Inherits from `SolichListView` to manage the listing of Google Cloud credentials with actions like edit and delete.
...
@method_decorator(check_integration_enabled(app_name="solich_meet"), name="dispatch")
@method_decorator(
    permission_required("solich_meet.add_googlecloudcredential"), name="dispatch"
)
class GoogleCredentialsFormView(views.SolichFormView):
    """
    View for creating or updating Google Cloud credentials.

    Inherits from `SolichFormView` to manage the form for creating or updating Google Cloud credentials.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("solich_meet.add_googlemeeting"), name="dispatch"
)
@method_decorator(check_integration_enabled(app_name="solich_meet"), name="dispatch")
class GmeetFormView(views.SolichFormView):
    """
    View for creating or updating Google Meet meetings.

    Inherits from `SolichFormView` to manage the form for creating or updating Google Meet meetings.
    Handles form validation, success, and failure responses. Displays success or error messages accordingly.
...
@method_decorator(check_integration_enabled(app_name="solich_meet"), name="dispatch")
@method_decorator(
    permission_required("solich_meet.view_googlemeeting"), name="dispatch"
)
class GmeetSectionView(views.SolichSectionView):
    """
    View for displaying the Google Meet section.

    Inherits from `SolichSectionView` to manage the section for Google Meet in the user interface.
...
class GmeetNavView(views.SolichNavView):
    """
    Nav view for Google Meet.

    Inherits from `SolichNavView` to manage nav bar for Google Meet view. Handles dynamic attributes for creating
...
class GmeetListView(views.SolichListView):
    """
    View for listing Google Meetings.

    Inherits from `SolichListView` to manage the listing of Google meetings with actions like edit, delete, and view meeting link.
...
class GmeetDetailedView(views.SolichDetailedView):
    """
    Detailed view for Google Meetings.

    Inherits from `SolichDetailedView` to manage the detailed view of a specific Google Meeting.
