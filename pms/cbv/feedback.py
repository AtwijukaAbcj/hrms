...
@method_decorator(login_required, name="dispatch")
class FeedbackListView(SolichListView):
    """
    list view
    """
...
@method_decorator(login_required, name="dispatch")
class FeedbackGenericTabView(SolichTabView):
    """
...
@method_decorator(login_required, name="dispatch")
class AnonymousFeedbackTab(SolichListView):
    """
...
@method_decorator(login_required, name="dispatch")
class FeedbacknavView(SolichNavView):
    """
    navbar
    """
...
@method_decorator(login_required, name="dispatch")
class AddAnonymousFeedbackForm(SolichFormView):
    """
    form view
    """
        ...
        if form.is_valid():
            if form.instance.pk:
                ...
                if feedback.based_on == "employee":
                    notify.send(
                        SolichUser.objects.filter(username="Solich Bot").first(),
...
class PerformanceTab(SelfFeedbacktab):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # SolichListView defaults selected_instances_key_id to the shared
...
class FeedbackUpdateFormView(SolichFormView):
    """
...
@method_decorator(login_required, name="dispatch")
class AnounyFeedbackDetailView(SolichDetailedView):
