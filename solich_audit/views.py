...
from solich_audit.forms import (
    AuditModelConfigForm,
    AuditModelFieldsForm,
    field_choices_for,
)
from solich_audit.models import AuditModelConfig
...
def audit_history_settings_view(request):
    ...
    """
    context = {}
    if request.user.has_perm("solich_audit.view_auditmodelconfig"):
        context.update(_audit_tracking_context())
    return render(request, "base/settings/audit_history.html", context)


@login_required
@permission_required("solich_audit.view_auditmodelconfig")
...
@login_required
@permission_required("solich_audit.change_auditmodelconfig")
...
def edit_audit_model_fields(request, pk):
    ...
    return render(
        request,
        "solich_audit/audit_model_fields_form.html",
