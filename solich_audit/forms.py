...
from solich_audit.models import AuditModelConfig, AuditTag, SolichAuditInfo
...
class HistoryForm(forms.Form):
    ...
    def as_history_modal(self, *args, **kwargs):
        ...
        """
        context = {"form": self}
        table_html = render_to_string("solich_audit/solich_audit_log.html", context)
        return table_html
...
def _eligible_models():
    """Return concrete SolichModel subclasses that can be audit-tracked."""
    ...
    for model in django_apps.get_models():
        if not issubclass(model, SolichModel) or model._meta.abstract:
            continue
...
def model_choices():
    from solich_audit.registry import DEFAULT_TRACKED_MODELS
