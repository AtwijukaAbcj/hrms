...
class LeaveTypeCondition(SolichModel):
    """
...
class LeaveRequest(SolichModel):
    ...
    def leave_type_custom(self):
        ...
        if apps.is_installed("recruitment"):
            Schedule = get_solich_model_class(
    ...
    def leave_requests_custom_emp_col(self):
        ...
        if apps.is_installed("recruitment"):
            ...
            if request is not None:
                cache = getattr(request, "_solich_interview_clash_cache", None)
                if cache is None:
                    cache = request._solich_interview_clash_cache = {}
    ...
    def is_approved(self):
        request = getattr(solich_middlewares._thread_locals, "request", None)
        if request:
            if not hasattr(request.user, "_solich_employee_cache"):
                request.user._solich_employee_cache = Employee.objects.filter(
                    employee_user_id=request.user
                ).first()
            employee = request.user._solich_employee_cache
...
if apps.is_installed("attendance"):
    ...
    class OverrideLeaveRequests(LeaveRequest):
        ...
        #     """
        #     WorkRecords = get_solich_model_class(
