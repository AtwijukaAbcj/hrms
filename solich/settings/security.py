...
Applied from base settings when DEBUG is False or SOLICH_ENV=production.
...
def is_production_mode(debug: bool, solich_env: str) -> bool:
    return (not debug) or (solich_env or "").strip().lower() == "production"
...
def validate_production_secrets(secret_key, allowed_hosts, db_init_password):
    ...
    if errors:
        raise ImproperlyConfigured(
            "Solich production security check failed:\n- " + "\n- ".join(errors)
        )
