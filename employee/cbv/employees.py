...
from solich.signals import post_generic_delete, post_generic_import
from solich_auth.models import SolichUser
from solich_views.cbv_methods import hx_request_required, login_required
from solich_views.forms import DynamicBulkUpdateForm
...
class EmployeesList(SolichListView):
    """
    ...
    import_related_model_column_mapping = {
        "employee_user_id": base_models.SolichUser,
...
@method_decorator(login_required, name="dispatch")
class TabEmployeeWorkList(SolichListView):
    """
...
@method_decorator(login_required, name="dispatch")
class EmployeeWorkDetails(SolichDetailedView):
    """
...
class EmployeeNav(SolichNavView):
    """
    For nav bar
    """
...
@receiver(post_generic_import, sender=SolichUser)
    ...
    def _set_password(records):
        ...
        if users_to_update:
            ...
            with transaction.atomic():
                SolichUser.objects.bulk_update(users_to_update, ["password"])
                logger.info(
...
class EmployeeCard(SolichCardView):
    """
    For card view
    """
