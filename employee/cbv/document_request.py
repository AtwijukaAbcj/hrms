...
from solich.decorators import manager_can_enter
from solich.http.response import SolichRedirect
from solich_documents.forms import DocumentForm
from solich_documents.forms import DocumentRejectCbvForm as RejectForm
from solich_documents.forms import DocumentRequestForm, DocumentUpdateForm
from solich_documents.models import Document, DocumentRequest
from solich_views import models as solich_views_models
from solich_views.cbv_methods import (
    hx_request_required,
    login_required,
    saved_filter_path_query,
)
from solich_views.generic.cbv.pipeline import Pipeline
from solich_views.generic.cbv.views import (
    SolichFormView,
    SolichListView,
    SolichNavView,
)
...
@method_decorator(
    manager_can_enter("solich_documents.add_documentrequest"), name="dispatch"
)
class DocumentRequestCreateForm(SolichFormView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        self.form = choosesubordinates(
            self.request, self.form, "solich_documents.add_documentrequest"
        )
...
@method_decorator(login_required, name="dispatch")
class DocumentCreateForm(SolichFormView):
    """
...
@method_decorator(login_required, name="dispatch")
@method_decorator(manager_can_enter("solich_documents.add_document"), name="dispatch")
class DocumentRejectCbvForm(SolichFormView):
    """
...
@method_decorator(login_required, name="dispatch")
class DocumentUploadForm(SolichFormView):
    """
...
@method_decorator(login_required, name="dispatch")
class DocumentRequestNav(SolichNavView):
    """
    For nav bar
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm(
            ...
            if self.request.user.has_perm(
                "solich_documents.change_documentrequest"
            ) or is_reportingmanager(self.request):
                self.actions = [
                    {
...
class DocumentRequestPipelineView(Pipeline):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Pipeline (unlike SolichListView) never builds filter_dict itself,
        ...
        # get_key_instances()-based construction SolichListView uses.
        ...
        # that every SolichListView/SolichCardView gets for free, so there
        ...
        context["stored_filters"] = solich_views_models.SavedFilter.objects.filter(
...
@method_decorator(login_required, name="dispatch")
class DocumentListView(SolichListView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        # DocumentRequestPipelineView. SolichListView always populates
        ...
        context["stored_filters"] = solich_views_models.SavedFilter.objects.none()
        return context
...
class DocumentIndividualTabList(DocumentListView):
    ...
    # SolichListView table template instead of one of those templates, that
    ...
    # - matching how every other SolichProfileView sub-tab in this framework
    ...
    def get_queryset(self, queryset=None, filtered=False, *args, **kwargs):
        ...
        # straight to SolichListView.get_queryset() with our own base
        ...
        return SolichListView.get_queryset(self, queryset, filtered, *args, **kwargs)
