# Solich Tour & Getting Started Checklist

Functional and technical reference for Solich’s onboarding UX:

1. **Product Tours** (`solich_tour`) — guided, driver.js walkthroughs on any page
2. **Setup Checklist** — “Get started with Solich HR” banner on the dashboard (8 setup steps)
...
## Part 1 — Product Tours (`solich_tour`)
...
1. Authenticated page load injects `window.SOLICH_TOUR` and `tourController.js`
...
│    window.SOLICH_TOUR = { page, path, activeUrl, … }       │
...
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│  Models (SolichModel + SolichCompanyManager)              │
...
- Installed app: `solich_tour` (`solich/settings/base.py`)
- URLs: `path("", include("solich_tour.urls"))` in `solich/urls.py`
- Context processor: `solich_tour.context_processors.pending_tours_flag`
  - Exposes `tour_launcher_enabled`, `tour_has_pending` for the header Help button
- Settings menu: `solich_tour/sidebar.py` → “Product Tours / Manage Tours”
- Scripts: `solich_theme/.../footer_scripts.html` (authenticated only)
...
| `tour-nav` / `tour-list` | SolichNavView / SolichListView | `view_tour` |
| `tour-create-form` / `tour-update-form` | SolichFormView | `add_tour` / change via update |
| `tour-steps` / `tour-step-form` / `tour-step-delete` | HTMX panel | `view_tour` / `change_tour` |

Patterns: SolichListView row actions, HTMX modals, generic delete.

#### Frontend (`tourController.js`)

- Depends on global **driver.js** (`window.driver.js.driver`)
- Reads `window.SOLICH_TOUR`
- Builds driver steps from API payload
- On destroy: posts `completed` if last step reached, else `skipped`
- Exposes launcher UI (`#solichTourLauncher`) and `window.solichTour` for Start/Replay
- Header button `#tourLauncherBtn` toggles the panel

#### Permissions

Django model perms on `solich_tour.Tour`:
...
## Part 2 — Setup Checklist (“Get started with Solich HR”)

Dashboard onboarding banner that walks admins through foundational HR configuration. Not part of `solich_tour`, but the primary “first run” tour of the product.

### Functional overview

**Copy & UX**

- Title: **Get started with Solich HR**
...
`_exists_for_company` mirrors `SolichCompanyManager`: record counts if assigned to company **or** company FK/M2M is null (shared lookup data).
...
|---|---|---|
| App | `solich_tour` | `base` (+ dashboard) |
...
solich_theme/.../footer_scripts.html   SOLICH_TOUR bootstrap
solich_theme/.../header.html           Take a tour button
```
...
6. **Permissions** — grant `solich_tour.view_tour` (and change/add) only to admins who should author tours.
`)
