...
2. Settings admin CRUD (permission-gated) to author tours and their ordered
   steps, following the SolichSectionView + SolichNavView + SolichListView +
   SolichFormView pattern used elsewhere in Settings.
"""
...
from solich.decorators import permission_required
from solich_tour.filters import TourFilter
from solich_tour.forms import TourForm, TourStepForm
from solich_tour.models import Tour, TourProgress, TourStep
...
@auth_login_required
@permission_required("solich_tour.view_tour")
def tour_settings_page(request):
    """Render the Tour Management page inside the Settings shell."""
    return render(request, "solich_tour/tour_settings.html")


@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("solich_tour.view_tour"), name="dispatch")
class TourNav(SolichNavView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm("solich_tour.add_tour"):
            self.create_attrs = f"""
...
class TourList(SolichListView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm("solich_tour.change_tour"):
            self.actions.insert(
                0,
                {
                    "action": _("Edit"),
        ...
        if self.request.user.has_perm("solich_tour.delete_tour"):
            self.actions.append(
                {
                    "action": _("Delete"),
                    "icon": "trash-outline",
                    "attrs": """
                        ...
                        hx-get="{get_delete_url}?model=solich_tour.tour&pk={pk}"
                        data-toggle="oh-modal-toggle"
...
@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("solich_tour.add_tour"), name="dispatch")
class TourFormView(SolichFormView):
...
def _render_steps_panel(request, tour):
    return render(
        request,
        "solich_tour/steps_panel.html",
        {
            "tour": tour,
            "steps": tour.steps.all(),
            "can_edit": request.user.has_perm("solich_tour.change_tour"),
        },
    )
...
@auth_login_required
@permission_required("solich_tour.change_tour")
