...
from solich_auth.models import (
    AuthUserGroups,
...
class Command(BaseCommand):
    help = "Migrate users from LegacyUser (auth_user) to SolichUser, including groups and permissions."
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for old_user in LegacyUser.objects.all():
                if SolichUser.objects.filter(username=old_user.username).exists():
                    skipped_count += 1
                    continue
                ...
                new_user = SolichUser.objects.create(
                    id=old_user.id,
