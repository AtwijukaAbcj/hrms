"""
Models for the solich_theme app
"""

from django.core.exceptions import ValidationError
from django.core.validators import RegexValidator

# Create your solich_theme models here.
...
class SolichColorTheme(SolichModel):
    """
    Model to store predefined color themes for Solich
    """
    ...
    class Meta:
        """
        Meta option for SolichColorTheme model
        """
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            if self.is_default:
                query = SolichColorTheme.objects.filter(is_default=True)
                if self.pk:
                    query = query.exclude(pk=self.pk)
                query.update(is_default=False)
            super().save(*args, **kwargs)
            if self.is_default and self.pk:
                SolichColorTheme.objects.filter(is_default=True).exclude(
                    pk=self.pk
...
class CompanyTheme(SolichModel):
    """
    ...
    def get_default_theme(cls):
        ...
        Returns the theme object or None
        """
        return SolichColorTheme.get_default_theme()
    ...
    def get_theme_for_company(cls, company):
        ...
        # return SolichColorTheme.objects.filter(
        #     name="Coral Red Theme (Default)"
        # ).first()
        return SolichColorTheme.objects.filter(is_default=True).first()
