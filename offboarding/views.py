...
def update_stage_order(request, pk):
    ...
    if not offboarding:
        return SolichRedirect(request, message=_("Offboarding not found"))
...
def delete_employee(request):
    ...
    if instances:
        ...
        notify.send(
            request.user.employee_get,
            recipient=SolichUser.objects.filter(
...
def add_note(request):
    ...
    if not employee:
        return SolichRedirect(request, message=_("Employee not found."))
    form = NoteForm()
    if request.method == "POST":
...
def update_task_status(request, *args, **kwargs):
    ...
    if not stage:
        return SolichRedirect(request, message=_("Stage not found"))
    stage_forms = {}
    stage_forms[str(stage.offboarding_id.id)] = StageSelectForm(
        offboarding=stage.offboarding_id
    )
...
def task_assign(request):
    ...
    if not task:
        return SolichRedirect(request, message=_("Task not found"))
    for employee in employees:
        try:
...
def request_single_view(request, id):
    ...
    if not letter:
        return SolichRedirect(request, message=_("Resignation letter not found"))
    context = {
        "letter": letter,
    }
    requests_ids_json = request.GET.get("requests_ids")
...
def resignation_list_swap_response(original_request):
    ...
    Subrequest keeps session (Solich CACHE filters) without relying on client-side JS reload.
    """
...
def update_status(request):
    ...
    contract_notice_end_date = (
        get_solich_model_class(app_label="payroll", model="contract")
...
def get_notice_period(request):
    ...
    employee_id = request.GET.get("employee_id")
    if apps.is_installed("payroll"):
        Contract = get_solich_model_class(app_label="payroll", model="contract")
        employee_contract = (
            (
...
def offboarding_dashboard(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_solich_model_class("recruitment", "candidate")
...
if apps.is_installed("pms"):
    ...
    def dashboard_feedback_table(request):
        ...
        Feedback = get_solich_model_class(app_label="pms", model="feedback")
